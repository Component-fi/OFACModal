...
export const useOfac = (address: string, ofacAddressesUrl: string = OFAC_JSON_LIST_URL): {isLoading: boolean, isBanned: boolean} => {
    ...
    useEffect(() => {
        getLowerCaseOfacAddresses(ofacAddressesUrl).then((_lowerCaseOfacAddresses: string[]) => {
...
const getLowerCaseOfacAddresses = async (ofacAddressesUrl: string): Promise<string[]> => {
    const response = await fetch(ofacAddressesUrl);
